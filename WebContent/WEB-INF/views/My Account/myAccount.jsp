...
<body>
	...
	ArrayList<AuctionModel> auctions = bid.selectAllBidByCID(person.getCid());
	%>
	
    <hgroup class="mb20">
        <h1>My eBids</h1>
        <h2 class="lead"> Toatal Number of Auction Placed: <strong class="text-danger"><%=auctions.size() %></strong></h2>
    </hgroup>
    <section class="col-xs-12 col-sm-6 col-md-12">
	<%for(AuctionModel result : auctions){ %>
		<%ProductModel product = bid.selectByID(result.getPid()); 
		  String img[] = bid.getPicturesByPid(result.getPid());
		%>
        <article class="search-result row">
            <div class="col-xs-12 col-sm-12 col-md-3">
                <a href="<%out.print("view?pid="+product.getPid());%>" class="thumbnail"><img src="<%out.print(img[0]); %>" alt="Picture will avilable soon" /></a>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-2">
                <ul class="meta-search">
                    <li><i class="glyphicon glyphicon-calendar"></i> <span><%=result.getBidDate() %></span></li>
                    <li><i class="glyphicon glyphicon-time"></i> 
                    <span id="<%out.print("s"+result.getBidId());%>">
                    <script>countDown("<%out.print(product.getEndDate());%>","<%out.print("s"+result.getBidId());%>");</script>
                    </span></li>
            ...
            <div class="col-xs-12 col-sm-12 col-md-7 excerpet">
                <h3><a href="<%out.print("view?pid="+product.getPid());%>" title=""><%=product.getPname() %></a></h3>
              	<strong>Rs.<%=result.getBidPrice() %></strong>
