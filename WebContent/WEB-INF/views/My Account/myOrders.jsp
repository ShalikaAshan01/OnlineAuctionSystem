...
<%
    ...
    if(null ==session.getAttribute("loggedinUser")){
    	session.setAttribute("from", "myorders");
...
<body>
	<!-- HEADER -->
		...
		<div class="container">
	<%
	ProductService orders = new ProductServiceImpl();
	List<Integer> productidlist = orders.myOrders(person.getCid());
	ArrayList<ProductModel> orderedProduct = new ArrayList<>();
	for(int pid:productidlist){
		ProductModel productObj = orders.selectByID(pid);
		orderedProduct.add(productObj);
	}
	int count = 0;
	%>
    ...
    <hgroup class="mb20">
        <h1>My eBids</h1>
        <h2 class="lead"> Toatal Number of Orders: <strong class="text-danger"><%=orderedProduct.size() %></strong></h2>
    </hgroup>
    <section class="col-xs-12 col-sm-6 col-md-12">
	<%for(ProductModel product: orderedProduct){ %>
